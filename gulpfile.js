...
function css() {
  ...
  return gulp
    ...
    .pipe(
      sass({
        includePaths: ['node_modules/uswds/dist/scss', 'node_modules/uswds/dist/scss/packages', '_scss'],
...
function jekyll(done) {
  ...
  if (env === 'prod') {
    ...
    return cp
      .spawn('bundle', ['exec', 'jekyll', 'build', '--config', '_config.yml,_config-dev.yml'], { stdio: 'inherit' })
      .on('close', done);
  }
}
...
const watch = gulp.series(clean, gulp.parallel(css, jekyll), gulp.parallel(watchFiles, browserSync));
